...
namespace cardiohealthv001
{
    ...
    public class DatabasePDetails
    {
        ...
        private List<PatientMessages> messagelist;
        ...
        {
            DatabasePAccount logininfo = new DatabasePAccount();
        ...
        public IList<PatientMessages> GetAllMessages(string username)
        {
            if (CheckUserName(username))
            {
                messagelist = new List<PatientMessages>();
                details = patientdetails.Collection.Find(Query.EQ("PUserName", username)).SetFields(Fields.Exclude("PUserName", "Name", "Email", "Address", "DateOfBirth", "PhoneNumber", "FitnessLevel", "Condition", "WeightKG", "IsMale", "Workouts", "Diagnosis")).Single();
                messagelist =  details.Messages.OrderBy(c => c.MessageNum).ToList();
                return messagelist;
            }
            else
            {
